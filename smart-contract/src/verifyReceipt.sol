...
contract verifyReceipt {
    ...
    function createReceipt(string calldata _receiptCid, string calldata _ctCid) public { // use calldata because we don't need to modify parameter if want to modify use memory instead
        ...
        fork.push(structRecord({receipt_cid: _receiptCid, ct_cid: _ctCid, timestamp: block.timestamp}));
